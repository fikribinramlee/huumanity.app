...
6. Do not use the "not X... but Y" revelation pivot in any form. This is one of the most common AI writing patterns: negate the expected framing ("it's not X", "the problem isn't A", "not because of X"), then pivot to a supposedly deeper truth ("it's Y", "but B", "but because Y"). The structure always pretends to reveal a hidden insight by dismissing the obvious one first. It shows up as: "It's not a milestone, it's PROOF that..." — "not because of some feel-good bullshit, it's because it PROVES..." — "the problem isn't A, but B" — "what works isn't X, but Y" — "it's not about the money, it's about the mission" — "this isn't about leads, it's about trust." All variations are banned, whether separated by a comma, a period, an em dash, or "but." The fix is always the same: drop the negation entirely and just say the point directly. Instead of "it's not a milestone, it's proof the problem is real" → write "the problem is real, and this proves it." Instead of "not because of feel-good bullshit, but because it proves your idea works" → write "it proves your idea actually works." Make the claim outright. Never set it up by first dismissing what it isn't.
